...
class Solution(Route):
    ...
    def add_point(self, h, point, position, route=None):
        if route is None:
            route = self.route
        routes = self.extract_routes(route)

        new_route = copy.deepcopy(route)
        routes[h] = list(routes[h][0:position]) + [point] + list(routes[h][position:])
        new_route[h]['route'] = routes[h]

        new_route[h]['time'] = self.route_time2(route_orig=route, h=h, route_new=new_route, position=position)

        return new_route

    def swap_point(self, h, point, position, route=None):
        ...
        new_route[h]['route'] = routes[h]
        new_route[h]['time'] = self.route_time3(route, h, new_route, position)
    ...
    def neighboorhood2(self, points=None, indexes=None, n_points=10, n_indexes=5, h=None, route=None):
        start = time.time()
        if route is None:
            route = self.route
        ...
        for h in h2:
            ...
            while seguir and len(fill2) > 0:
                ...
                for p in list(points2):
                    for i in np.random.choice(indexes[h], size=min(n_indexes, len(indexes[h])), replace=False):
                        try:
                            y = mi.add_point(h, p, i, route)
                            for id, ab in enumerate(y):
                                if id == h:
                                    y[id]['fill_level'] = fill[p]
                                else:
                                    y[id]['fill_level'] = route[id]['fill_level']
                            routes.append(y)
                        except:
                            0
                    if len(route[h]['route']) > 2:
                        try:
                            y = mi.swap_point(h, p, i, route)
                            for id, ab in enumerate(y):
                                if id == h:
                                    y[id]['fill_level'] = fill[p]
                                else:
                            ...
                            0
                routes = mi.route_time_constrain(routes)
                if len(routes) > 0:
                    seguir = False
    ...
    def best_neighbor(self, routes, k=None, vecino_bueno=None):
...
tabu.max_tabu = 3
tabu.max_tabu_h = 2
tabu.optimize(100, n_points=1000, n_indexes=200, indexes=None, print_each=1)
