...
class waste_collection:
    ...
    def read_filling_rates(self, file_filling_rates):
        ...
        fill_rate.update(dict.fromkeys(self.ini_points, 0))
    ...
    def read_times(self, file_times):
        ...
        times0 = {'origen': self.ini_points, 'destino': self.ini_points, 'tiempo': 0}
        times0 = pd.DataFrame(times0)
...
#         return collection
#
#     def collection_days(self, route):
#         l = dict.fromkeys(chain(*route), [])
#         for h, r in enumerate(route):
#             for c in r:
#                 l[c] = l[c] + [h]
#         return l
#
#     def days_since_last_collection(self, days):
#         d = []
#         for i in days.values():
#             if len(i) >= 2:
#                 d.append([i[0]] + [i[k] - i[k - 1] for k in range(1, len(i))])
#             else:
#                 d.append(i)
#         return d
#
#
#     def fill_collected(self, route):
#         days = self.collection_days(route)
#
#         fill_ini = 0.25
#         fill_rate = 0.10
#         collected = [0] * self.horizon
#         for p in days.keys():
#             for i, d in enumerate(a2[p]):
#                 if i == 0:
#                     mia = fill_ini + d * fill_rate
#                 else:
#                     mia = fill_rate * (days[p][i] - days[p][i - 1])
#                 collected[d] += mia
#         return collected
#
#
...
#     if len(i) >= 2:
#         print(i)
#         ja.append([i[0]] + [i[k] - i[k-1] for k in range(1, len(i))])
#     else:
#         ja.append(i)
#
#
#  def collection_days(route):
#         l = dict.fromkeys(chain(*route), [])
#         for h, r in enumerate(route):
#             for c in r:
#                 l[c] = l[c] + [h]
